...
def open_team_search(cnx, cur, root, window):
    # root.withdraw()
    window.withdraw()
    # Create new window
    wn = Toplevel(root)
    ...
    cur.execute('CALL end_of_season_team_drop_down()')

    for row in cur.fetchall():
        team_drop_down.append(str(row['type']))
    ...
    Label = customtkinter.CTkLabel(frame_filters, text="End of Season Teams Search:")
...
def run_search(cur, year, t_name, treeview):
    ...
    for row in cur.fetchall():
        new_row = {'team': row['type'], 'player_name': row['player'],
...
def get_command(input, year, position, p_name, t_name):
    if input == '':
        return 'CALL generic_eos_search()'
    elif input == 'Y':
        return f'CALL eos_search_year({year})'
    elif input == 'T':
        return f'CALL eos_search_team("{t_name}")'
    else:
        return f'CALL eos_search_year_team({year}, "{t_name}")'
